...
import {
  ...
  GripVertical, Lock, Unlock, Calendar, Pencil, Trash2, ArrowRight, List, LayoutGrid,
} from 'lucide-react';
...
export function PlannedView({ posts, onUpdatePost, onEditPost, onDeletePost }: Props) {
  ...
  function commitReorder(target: string) {
    ...
    newOrder.forEach((p, i) => { if (!p.locked) onUpdatePost({ ...p, planOrder: i * 1000 }); });
    endDrag();
  }

  // ── List drag handlers (vertical) ──────────────────────────────────────────
  function startListDrag(e: React.DragEvent, id: string) {
    dragNode.current = e.currentTarget as HTMLDivElement;
    lastInsertRef.current = null;
    setDragId(id); setInsertAt(null);
    e.dataTransfer.effectAllowed = 'move';
    setTimeout(() => { if (dragNode.current) dragNode.current.style.opacity = '0.3'; }, 0);
  }
  function handleListDragOver(e: React.DragEvent, id: string) {
    e.preventDefault();
    const rect = (e.currentTarget as HTMLElement).getBoundingClientRect();
    const t = e.clientY < rect.top + rect.height / 2 ? id : `after:${id}`;
  ...
  // ── Grid drag handlers (horizontal) ────────────────────────────────────────
  function startGridDrag(e: React.DragEvent, id: string) {
    dragNode.current = e.currentTarget as HTMLDivElement;
    lastInsertRef.current = null;
    setDragId(id); setInsertAt(null);
    e.dataTransfer.effectAllowed = 'move';
  ...
  function handleGridDragOver(e: React.DragEvent, id: string) {
    e.preventDefault();
    const rect = (e.currentTarget as HTMLElement).getBoundingClientRect();
    const t = e.clientX < rect.left + rect.width / 2 ? id : `after:${id}`;
    setInsertAt(t); lastInsertRef.current = t;
  }
  function handleGridDrop(e: React.DragEvent, id: string) {
    e.preventDefault();
    const rect = (e.currentTarget as HTMLElement).getBoundingClientRect();
    const t = insertAt ?? lastInsertRef.current ?? (e.clientX < rect.left + rect.width / 2 ? id : `after:${id}`);
    commitReorder(t);
  }
      ...
      <div style={{ position: 'sticky', top: 56, zIndex: 20, background: C.bg, paddingTop: 12, paddingBottom: 12, marginBottom: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, borderBottom: `1px solid ${C.line}`, marginLeft: -24, marginRight: -24, paddingLeft: 24, paddingRight: 24 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: C.t2, marginTop: 3, marginBottom: 0 }}>
            {planned.length} posts · drag to set order · lock to pin
          </p>
        </div>
      ...
      {viewMode === 'list' && (
        <div onDragOver={e => e.preventDefault()} onDrop={handleListDrop}>
          {planned.map((post, idx) => {
            ...
            const showAbove = insertAt === post.id && dragId !== post.id;
            const showBelow = insertAt === `after:${post.id}` && dragId !== post.id;
            ...
            return (
              <div key={post.id}>
                ...
                <div
                  ...
                  onDragStart={post.locked ? undefined : e => startListDrag(e, post.id)}
                  onDragEnd={endDrag}
                  onDragOver={e => handleListDragOver(e, post.id)}
                  onDragLeave={e => { if (!e.currentTarget.contains(e.relatedTarget as Node)) setInsertAt(null); }}
                  onDrop={handleListDrop}
                  style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', marginBottom: 6, background: '#fff', borderRadius: 12, border: post.locked ? `2px solid #F59E0B` : `1px solid ${C.line}`, boxShadow: isDragging ? 'none' : '0 1px 4px rgba(0,0,0,0.06)', opacity: isDragging ? 0.3 : 1, cursor: post.locked ? 'default' : dragId ? 'grabbing' : 'grab', transition: 'opacity 0.15s' }}
                >
                  ...
                  {post.locked ? <Lock size={13} color="#F59E0B" style={{ flexShrink: 0 }} /> : <GripVertical size={15} color={C.t3} style={{ flexShrink: 0 }} />}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: 13, color: C.t1, margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
                    ...
                    <button onClick={() => onDeletePost(post.id)} style={{ width: 28, height: 28, borderRadius: 7, border: `1px solid ${C.line}`, background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#EF4444' }}>
                      <Trash2 size={13} />
                    </button>
                  </div>
                </div>
          ...
          {planned.length === 0 && <div style={{ textAlign: 'center', padding: '80px 0', color: C.t3, fontSize: 14 }}>No posts yet — create one and it'll appear here</div>}
        </div>
      )}
        ...
        <div style={{ transformOrigin: 'top left', transform: `scale(${zoom / 100})`, width: `${10000 / zoom}%`, transition: 'transform 0.15s' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 3, background: '#E5E7EB', border: '3px solid #E5E7EB', borderRadius: 4 }}>
            {planned.map((post, idx) => {
              const Icon = PLATFORM_ICON[post.platform];
              const isDragging = dragId === post.id;
              const isHovered  = hoverKey === post.id;
              const insB = insertAt === post.id && !isDragging;
              const insA = insertAt === `after:${post.id}` && !isDragging;
              const hasDate = post.scheduledDate && post.status === 'scheduled';
              return (
                <div key={post.id} style={{ position: 'relative' }}>
                  ...
                  <div
                    draggable={!post.locked}
                    onDragStart={post.locked ? undefined : e => startGridDrag(e, post.id)}
                    onDragEnd={endDrag}
                    onDragOver={e => handleGridDragOver(e, post.id)}
                    onDragLeave={e => { if (!e.currentTarget.contains(e.relatedTarget as Node)) setInsertAt(null); }}
                    onDrop={e => handleGridDrop(e, post.id)}
                    onMouseEnter={() => setHoverKey(post.id)}
                    onMouseLeave={() => setHoverKey(null)}
                    style={{ aspectRatio: '1', overflow: 'hidden', position: 'relative', cursor: post.locked ? 'default' : dragId ? 'grabbing' : 'grab', background: C.bg, opacity: isDragging ? 0.25 : 1, transition: 'opacity 0.15s', outline: post.locked ? '2px solid #F59E0B' : 'none', outlineOffset: '-2px' }}
                  >
                    ...
                    {/* Platform dot */}
                    <div style={{ position: 'absolute', top: 5, right: 5, width: 8, height: 8, borderRadius: '50%', background: PLATFORM_COLOR[post.platform], border: '1.5px solid rgba(255,255,255,0.9)', boxShadow: '0 1px 4px rgba(0,0,0,0.3)' }} />
                    ...
                    {/* Date badge */}
                    {hasDate && (
                      <div style={{ position: 'absolute', bottom: 6, right: 6, background: 'rgba(0,0,0,0.55)', borderRadius: 4, padding: '2px 5px', fontSize: 9, fontWeight: 600, color: '#fff', lineHeight: 1.3 }}>
                        {post.scheduledDate.toLocaleDateString('en-AU', { day: 'numeric', month: 'short' })}
                      </div>
                    )}
                    ...
                    {isHovered && !isDragging && (
                      <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 5, padding: 6 }}>
                        {!post.locked && <GripVertical size={16} color="#fff" />}
                        ...
                        <div style={{ display: 'flex', gap: 4, marginTop: 2 }}>
                          {/* Lock toggle */}
                          <button onClick={e => { e.stopPropagation(); onUpdatePost({ ...post, locked: !post.locked }); }} style={{ background: post.locked ? '#F59E0B' : 'rgba(255,255,255,0.2)', border: 'none', borderRadius: 5, padding: '3px 6px', display: 'flex', alignItems: 'center', gap: 3, cursor: 'pointer', color: '#fff', fontSize: 9, fontWeight: 600 }}>
                        ...
                        {/* Inline date picker in grid */}
                        {datePickerId === post.id && (
                          <input autoFocus type="datetime-local" defaultValue={toLocalInputValue(post.scheduledDate)}
                            onChange={e => { if (!e.target.value) return; onUpdatePost({ ...post, scheduledDate: new Date(e.target.value), status: 'scheduled' }); }}
            ...
            {Array.from({ length: planned.length % 3 === 0 ? 0 : 3 - (planned.length % 3) }).map((_, i) => (
